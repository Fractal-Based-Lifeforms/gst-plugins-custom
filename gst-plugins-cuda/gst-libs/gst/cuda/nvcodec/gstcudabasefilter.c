...
G_DEFINE_ABSTRACT_TYPE(
    GstCudaBaseFilter,
    gst_cuda_base_filter,
    GST_TYPE_CUDA_BASE_TRANSFORM);

static void gst_cuda_base_filter_dispose(GObject *object);
static GstFlowReturn gst_cuda_base_filter_transform_frame(
    GstCudaBaseTransform *btrans,
    GstVideoFrame *in_frame,
    GstCudaMemory *in_cuda_mem,
    GstVideoFrame *out_frame,
    GstCudaMemory *out_cuda_mem);
static gboolean gst_cuda_base_filter_set_info(
    GstCudaBaseTransform *btrans,
    GstCaps *incaps,
...
static void gst_cuda_base_filter_class_init(GstCudaBaseFilterClass *klass)
{
    ...
    GST_DEBUG_CATEGORY_INIT(
        gst_cuda_base_filter_debug, "cudabasefilter", 0, "CUDA Base Filter");
}

static void gst_cuda_base_filter_init(GstCudaBaseFilter *convert)
{
}
...
static gboolean gst_cuda_base_filter_configure(
    GstCudaBaseFilter *filter,
    GstVideoInfo *in_info,
    GstVideoInfo *out_info)
{
    ...
    GstCudaMemory *out_cuda_mem)
{
    ...
    else
    {
        if(!filter->in_fallback)
        {
            ...
            filter->in_fallback = (GstCudaMemory *)gst_cuda_allocator_alloc(
                filter->allocator, GST_VIDEO_INFO_SIZE(&params.info), &params);
        }
        ...
        {
            GST_ELEMENT_ERROR(
                filter, LIBRARY, FAILED, (NULL), ("Cannot push CUDA context"));
            return FALSE;
        }
    ...
    else
    {
        if(!filter->out_fallback)
        {
            ...
            filter->out_fallback = (GstCudaMemory *)gst_cuda_allocator_alloc(
                filter->allocator, GST_VIDEO_INFO_SIZE(&params.info), &params);
        }
    ...
    conv_ret = gst_cuda_converter_frame(
        filter->converter,
        in_mem,
        &btrans->in_info,
        out_mem,
