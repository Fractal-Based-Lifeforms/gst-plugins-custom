...
GST_DEBUG_CATEGORY_STATIC(gst_cudaloader_debug);
...
#define LOAD_SYMBOL(name, func)                                       \
    G_STMT_START                                                      \
    {                                                                 \
        if(!g_module_symbol(                                          \
               module, G_STRINGIFY(name), (gpointer *)&vtable->func)) \
        {                                                             \
            GST_ERROR(                                                \
...
typedef struct _GstNvCodecCudaVTable
{
    ...
    CUresult(CUDAAPI *CuCtxEnablePeerAccess)(
        CUcontext peerContext,
        unsigned int Flags);
    CUresult(CUDAAPI *CuCtxDisablePeerAccess)(CUcontext peerContext);
    CUresult(CUDAAPI *CuGraphicsMapResources)(
        unsigned int count,
        CUgraphicsResource *resources,
        CUstream hStream);
    CUresult(CUDAAPI *CuGraphicsUnmapResources)(
        unsigned int count,
        CUgraphicsResource *resources,
        CUstream hStream);
    ...
    CUresult(CUDAAPI *CuGraphicsResourceGetMappedPointer)(
        ...
        size_t *pSize,
        CUgraphicsResource resource);
    CUresult(CUDAAPI *CuGraphicsUnregisterResource)(
        CUgraphicsResource resource);
    ...
    CUresult(
        CUDAAPI *CuMemcpy2DAsync)(const CUDA_MEMCPY2D *pCopy, CUstream hStream);
    ...
    CUresult(CUDAAPI *CuDeviceGetAttribute)(
        ...
        CUdevice dev);
    CUresult(CUDAAPI *CuDeviceCanAccessPeer)(
        int *canAccessPeer,
        CUdevice dev,
        CUdevice peerDev);
    ...
    CUresult(CUDAAPI *CuModuleGetFunction)(
        CUfunction *hfunc,
        CUmodule hmod,
        const char *name);
    CUresult(CUDAAPI *CuTexObjectCreate)(
    ...
    CUresult(CUDAAPI *CuGraphicsGLRegisterImage)(
        CUgraphicsResource *pCudaResource,
        unsigned int image,
        unsigned int target,
        unsigned int Flags);
    CUresult(CUDAAPI *CuGraphicsGLRegisterBuffer)(
        CUgraphicsResource *pCudaResource,
        unsigned int buffer,
        unsigned int Flags);
    CUresult(CUDAAPI *CuGraphicsResourceSetMapFlags)(
        CUgraphicsResource resource,
        unsigned int flags);
    CUresult(CUDAAPI *CuGLGetDevices)(
...
static GstNvCodecCudaVTable gst_cuda_vtable = {
    0,
};
...
gboolean gst_cuda_load_library(void)
{
    ...
    GST_DEBUG_CATEGORY_INIT(
        gst_cudaloader_debug, "cudaloader", 0, "CUDA Dynamic Library Loader");
    ...
    {
        GST_WARNING(
            "Could not open library %s, %s", filename, g_module_error());
        return FALSE;
    }
    ...
    unsigned int mipLevel)
{
    ...
    return gst_cuda_vtable.CuGraphicsSubResourceGetMappedArray(
    ...
    CUgraphicsResource resource)
{
    ...
    return gst_cuda_vtable.CuGraphicsResourceGetMappedPointer(
    ...
    unsigned int ElementSizeBytes)
{
    ...
    return gst_cuda_vtable.CuMemAllocPitch(
    ...
    const CUDA_RESOURCE_VIEW_DESC *pResViewDesc)
{
    ...
    return gst_cuda_vtable.CuTexObjectCreate(
    ...
    void **extra)
{
    ...
    return gst_cuda_vtable.CuLaunchKernel(
        f,
        gridDimX,
        gridDimY,
        gridDimZ,
    ...
    unsigned int Flags)
{
    ...
    return gst_cuda_vtable.CuGraphicsGLRegisterImage(
    ...
    unsigned int Flags)
{
    ...
    return gst_cuda_vtable.CuGraphicsGLRegisterBuffer(
    ...
    CUGLDeviceList deviceList)
{
    ...
    return gst_cuda_vtable.CuGLGetDevices(
