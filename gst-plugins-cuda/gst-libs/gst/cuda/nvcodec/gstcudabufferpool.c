...
G_DEFINE_TYPE_WITH_PRIVATE(
    GstCudaBufferPool,
    gst_cuda_buffer_pool,
    GST_TYPE_BUFFER_POOL);

static const gchar **gst_cuda_buffer_pool_get_options(GstBufferPool *pool)
{
...
gst_cuda_buffer_pool_set_config(GstBufferPool *pool, GstStructure *config)
{
    ...
    if(!gst_buffer_pool_config_get_params(
           config, &caps, &size, &min_buffers, &max_buffers))
        goto wrong_config;
    ...
    gst_buffer_pool_config_set_params(
        config, caps, GST_VIDEO_INFO_SIZE(info), min_buffers, max_buffers);
...
wrong_caps:
    {
        GST_WARNING_OBJECT(
            pool, "failed getting geometry from caps %" GST_PTR_FORMAT, caps);
        return FALSE;
    }
no_allocator:
    {
...
static GstFlowReturn gst_cuda_buffer_pool_alloc(
    GstBufferPool *pool,
    GstBuffer **buffer,
    GstBufferPoolAcquireParams *params)
{
    ...
    mem = gst_cuda_allocator_alloc(
        GST_ALLOCATOR_CAST(priv->allocator),
...
static void gst_cuda_buffer_pool_dispose(GObject *object)
{
...
static void gst_cuda_buffer_pool_class_init(GstCudaBufferPoolClass *klass)
{
    ...
    GST_DEBUG_CATEGORY_INIT(
        gst_cuda_buffer_pool_debug, "cudabufferpool", 0, "CUDA Buffer Pool");
}

static void gst_cuda_buffer_pool_init(GstCudaBufferPool *pool)
{
